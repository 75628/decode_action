// == QingLong standalone wxapp script ==
// Generated from smallfawn/QLScriptPublic wxapp directory.
// Internal repo helpers were inlined or made optional so this file can run by itself in QingLong.

// QingLong standalone helper: use notify module when it exists, otherwise skip notification.
function loadQingLongNotify() {
    if (typeof require !== "function") return null;
    const candidates = ["./sendNotify.js", "./sendNotify", "../sendNotify.js", "../sendNotify", "sendNotify"];
    for (const name of candidates) {
        try {
            return require(name);
        } catch (e) {
            if (e && e.code && e.code !== "MODULE_NOT_FOUND") {
                console.error(`加载通知模块 ${name} 失败: ${e.message || e}`);
            }
        }
    }
    return null;
...
const notify = $.isNode() ? loadQingLongNotify() : '';
let ckName = "wx_midea";
let envSplitor = ["&", "\n"]; //多账号分隔符
